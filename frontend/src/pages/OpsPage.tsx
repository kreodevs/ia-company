...
export default function OpsPage() {
  ...
  if (loading) {
    return <p className="text-[var(--color-muted-foreground)]">Loading operations dashboard…</p>;
  }

  if (!portfolio) {
    return <p className="text-[var(--color-muted-foreground)]">Unable to load portfolio.</p>;
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-muted-foreground)]">
            Multi-product portfolio — autonomous meta-orchestrator picks the next workflow each cycle.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          {metaSchedule && (
            ...
            >
              {runningMeta ? "Starting…" : "Run meta cycle now"}
            </button>
          )}
          <Link
            to="/consensus"
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Company phase" value={PHASE_LABELS[portfolio.companyPhase] ?? portfolio.companyPhase} hint={`Cycle #${portfolio.cycleNumber}`} />
        <StatCard label="Products building" value={portfolio.stats.building} hint={`${portfolio.stats.growing} growing`} />
        <StatCard label="Pipeline ideas" value={portfolio.stats.pipeline} />
        <StatCard label="Revenue tracked" value={`$${portfolio.stats.totalRevenueUsd.toFixed(0)}`} />
      </section>

      {(portfolio.nextAction || nextRun) && (
        <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-5">
          <h2 className="font-semibold">Next autonomous action</h2>
          {portfolio.nextAction && (
            <p className="mt-2 text-sm">{portfolio.nextAction}</p>
          )}
          {nextRun && (
            <div className="mt-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-background)] p-3 text-sm">
              <div>
                <span className="text-[var(--color-muted-foreground)]">Workflow:</span>{" "}
          ...
          {portfolio.focusProduct && (
            <p className="mt-2 text-xs text-[var(--color-muted-foreground)]">
              Focus product: <strong>{portfolio.focusProduct.name}</strong> ({portfolio.focusProduct.slug})
            </p>
          )}
        </section>
      )}
          ...
          <ul className="space-y-2">
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-[var(--color-muted-foreground)]">
                    ...
                    {product.revenueUsd > 0 && ` · $${product.revenueUsd.toFixed(0)} revenue`}
                  </div>
                </div>
            ...
            {portfolio.products.length === 0 && (
              <p className="text-sm text-[var(--color-muted-foreground)]">
                No products yet — run opportunity discovery to populate the pipeline.
              </p>
            )}
          </ul>
        </div>
        ...
        <div className="space-y-3">
          ...
          <ul className="space-y-2">
            {portfolio.pipeline.map((idea) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <PhaseBadge phase={idea.goNoGo === "go" ? "growing" : idea.goNoGo === "no_go" ? "paused" : "evaluating"} />
                </div>
                {idea.goNoGo === "pending" && (
            ...
            {portfolio.pipeline.length === 0 && (
              <p className="text-sm text-[var(--color-muted-foreground)]">
                Pipeline empty — discovery cycles add ideas here.
              </p>
            )}
          </ul>
        </div>
