  ...
  return (
    <div ref={rootRef} className={`relative ${className}`}>
      <input
        ...
        className="mt-1 w-full rounded-[var(--radius-inputs)] border border-[var(--color-hairline)] bg-[var(--color-paper-white)] px-3 py-2 text-sm"
        value={query}
      ...
      {selected ? (
        <p className="mt-1 text-xs text-[var(--color-fog-gray)]">
      ...
      {open && !loading && filtered.length > 0 ? (
        <ul
          id={listId}
          role="listbox"
          className="absolute z-20 mt-1 max-h-72 w-full overflow-y-auto rounded-[var(--radius-sm)] border border-[var(--color-hairline)] bg-[var(--color-paper-white)] shadow-none"
        >
          {filtered.map((model) => (
            <li key={model.id} role="option" aria-selected={model.id === value}>
              <button
                type="button"
                className="interactive flex w-full flex-col gap-0.5 border-b border-[var(--color-hairline)] px-3 py-2 text-left last:border-b-0 hover:bg-[var(--color-mist-white)]"
                onMouseDown={(e) => e.preventDefault()}
                onClick={() => commitValue(model.id)}
              >
                <span className="text-sm font-medium text-[var(--color-vault-ink)]">{model.id}</span>
                {model.name !== model.id ? (
                  <span className="text-xs text-[var(--color-fog-gray)]">{model.name}</span>
                ) : null}
                <span className="text-xs text-[var(--color-fog-gray)]">
                  {t("admin.platformSettings.defaultLlm.modelPricing", {
      ...
      {open && !loading && !error && filtered.length === 0 && query.trim() ? (
        <p className="absolute z-20 mt-1 w-full rounded-[var(--radius-sm)] border border-[var(--color-hairline)] bg-[var(--color-paper-white)] px-3 py-2 text-xs text-[var(--color-fog-gray)]">
