  ...
  (
    ...
    const validateFile = (file: File): string | null => {
      ...
      if (accept) {
        ...
        if (!isValid) return "Tipo de archivo no permitido";
      }
      return null;
    };
    ...
    const processFiles = useCallback(
      async (selectedFiles: FileList | File[]) => {
        ...
        const updatedFiles = multiple ? [...currentFiles, ...newUploadedFiles] : newUploadedFiles;
          ...
          for (const uploadedFile of newUploadedFiles) {
            try {
              ...
              await uploadFn(uploadedFile.file, (progress) => {
                const updated = updatedFiles.map((f) => (f.id === uploadedFile.id ? { ...f, progress } : f));
                updateFiles(updated);
              });
      ...
      <div ref={ref} className={`w-full ${className}`} data-testid="file-upload">
        <div
          ...
          className={`
            ...
            rounded-[var(--radius-lg)] border-2 border-dashed transition-all duration-[var(--transition-base)] cursor-pointer
            ${
              disabled
        ...
        >
          ...
          <div className="w-12 h-12 mb-[var(--spacing-md)] flex items-center justify-center rounded-full bg-[var(--accent)]/10 text-[var(--accent)]">
            <Upload className="w-6 h-6" />
          </div>

          <p className="text-sm text-[var(--foreground)] font-medium text-center">{dropzoneText}</p>
        ...
        {currentFiles.length > 0 && (
          <div className="mt-[var(--spacing-md)] space-y-[var(--spacing-sm)]" data-testid="file-list">
            {currentFiles.map((file) => (
              <div
                key={file.id}
                className="flex items-center gap-[var(--spacing-md)] p-[var(--spacing-md)] rounded-[var(--radius)] bg-[var(--card)] border border-[var(--card-border)]"
              >
                {file.preview ? (
                  <img src={file.preview} alt={file.name} className="w-10 h-10 rounded-[var(--radius-sm)] object-cover" />
                ) : (
                  <div className="w-10 h-10 flex items-center justify-center rounded-[var(--radius-sm)] bg-[var(--secondary)] text-[var(--foreground-muted)]">
                    {FileIcon(file.type)}
                  </div>
                )}
                ...
                <div className="flex items-center gap-[var(--spacing-sm)]">
                  {file.status === "success" && <CheckCircle className="w-5 h-5 text-[var(--success)]" />}
                  {file.status === "error" && <AlertCircle className="w-5 h-5 text-[var(--destructive)]" />}
                  <button
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    className="p-[var(--spacing-xs)] rounded-[var(--radius-sm)] text-[var(--foreground-muted)] hover:text-[var(--destructive)] hover:bg-[var(--destructive)]/10 transition-colors duration-[var(--transition-fast)]"
