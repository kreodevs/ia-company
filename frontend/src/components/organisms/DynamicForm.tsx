...
import { MarkdownEditorWysiwyg } from "../molecules/MarkdownEditorWysiwyg";
...
import { RoleFlagsGroup } from "../molecules/RoleFlagsGroup";
import { DEFAULT_LEGACY_ROLE_FLAGS } from "@/lib/rbac-mappers";
...
export const DynamicForm = ({
  ...
  if (loading) {
    const skeletonSections = sections.length > 0 ? sections.length : 1;
    return (
      <div className={cn("w-full space-y-[var(--spacing-xl)]", className)}>
        {Array.from({ length: skeletonSections }).map((_, sIdx) => (
          <div
            key={sIdx}
            className="col-span-12 bg-[var(--background)] border border-[var(--border)] rounded-2xl p-[var(--spacing-lg)] md:p-8 space-y-[var(--spacing-xl)] shadow-sm"
          >
            <div className="space-y-[var(--spacing-sm)] border-b border-[var(--border)]/60 pb-[var(--spacing-lg)] mb-[var(--spacing-lg)]">
              <div className="h-5 w-48 rounded bg-[var(--muted)] animate-pulse" />
              <div className="h-3.5 w-72 rounded bg-[var(--muted)] animate-pulse opacity-60" />
            </div>
            <div className="grid grid-cols-12 gap-x-[var(--spacing-lg)] gap-y-7">
              {Array.from({ length: 4 }).map((_, fIdx) => (
                <div key={fIdx} className="col-span-6 space-y-[var(--spacing-sm)]">
                  <div className="h-3 w-20 rounded bg-[var(--muted)] animate-pulse" />
                  <div className="h-10 w-full rounded-[var(--radius)] bg-[var(--muted)] animate-pulse" />
                  <div className="h-3 w-32 rounded bg-[var(--muted)] animate-pulse opacity-60" />
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex justify-end gap-[var(--spacing-md)]">
          {onCancel && <div className="h-10 w-24 rounded-[var(--radius)] bg-[var(--muted)] animate-pulse" />}
          <div className="h-10 w-[140px] rounded-[var(--radius)] bg-[var(--muted)] animate-pulse" />
        </div>
      </div>
    );
  }

  const allFields = useMemo(() => (sections.length > 0 ? sections.flatMap((s) => s.fields) : fields), [fields, sections]);
  ...
  const formSchema = useMemo(() => {
    ...
    allFields.forEach((field) => {
      ...
      if (field.type === "number" || field.type === "currency" || field.type === "rating") {
        ...
        validator = z.boolean();
        if (field.required) validator = validator.refine((val: boolean) => val === true, reqMessage);
      } else if (field.type === "role-flags") {
        validator = z.object({
          admin: z.boolean(),
          sales: z.boolean(),
          warehouse: z.boolean(),
  ...
  const defaultValues = useMemo(() => {
    ...
    allFields.forEach((f) => {
      ...
      } else if (f.type === "role-flags") {
        defaults[f.name] = f.defaultValue ?? DEFAULT_LEGACY_ROLE_FLAGS;
      } else if (["checkbox", "switch"].includes(f.type)) {
        defaults[f.name] = false;
      } else if (["multiselect", "tags", "file"].includes(f.type)) {
        defaults[f.name] = [];
      } else if (f.type === "date") {
  ...
  const renderInput = (field: DynamicFormField, props: any) => {
    ...
    switch (field.type) {
      ...
      case "password":
        return <Password placeholder={field.placeholder} {...props.field} className="w-full" />;
      case "number":
      ...
      case "checkbox":
        return (
          <div className="flex items-center gap-[var(--spacing-md)] p-[var(--spacing-sm)] rounded-md hover:bg-[var(--background-secondary)] transition-colors cursor-pointer border border-transparent hover:border-[var(--border)]">
            <Checkbox checked={Boolean(value)} onChange={(checked) => onChange(checked)} id={field.name} />
            <label htmlFor={field.name} className="text-sm font-medium leading-none cursor-pointer text-[var(--foreground)]">
      ...
      case "switch":
        return (
          <div className="flex items-center justify-between gap-[var(--spacing-md)] p-[var(--spacing-md)] rounded-lg bg-[var(--background-secondary)]/50 border border-[var(--border)] group hover:border-[var(--primary)]/30 transition-all">
            <label htmlFor={field.name} className="text-sm font-medium cursor-pointer text-[var(--foreground)] group-hover:text-[var(--primary)] transition-colors">
              {field.placeholder || field.label}
            </label>
      ...
      case "richtext":
        return (
          <div className="rounded-md border border-[var(--border)] overflow-hidden">
            <MarkdownEditorWysiwyg value={value ?? ""} onChange={onChange} placeholder={field.placeholder} minHeight="240px" />
          </div>
        );
      case "html-richtext":
        return (
          <div className="rounded-md border border-[var(--border)] overflow-hidden">
            <RichTextEditor value={value} onChange={(htmlValue) => onChange(htmlValue)} placeholder={field.placeholder} />
          </div>
        );
      case "tags":
      ...
      case "role-flags":
        return (
          <RoleFlagsGroup
            value={value ?? DEFAULT_LEGACY_ROLE_FLAGS}
            onChange={onChange}
            name={field.name}
            layout="stacked"
          />
  ...
  const renderField = (field: DynamicFormField, index: number) => {
    const isInteractive = ["switch", "checkbox", "color", "rating", "role-flags"].includes(field.type);
        ...
        <FormField
          ...
          render={(fieldProps) => (
            <FormItem className="space-y-[var(--spacing-sm)] animate-in fade-in slide-in-from-bottom-2 duration-300">
              {!["checkbox", "switch", "role-flags"].includes(field.type) && (
        ...
        >
          {sections.length > 0
            ? sections.map((section, idx) => (
                <div
                  key={section.id || idx}
                  className="col-span-12 bg-[var(--background)] border border-[var(--border)] rounded-2xl p-[var(--spacing-lg)] md:p-8 space-y-[var(--spacing-xl)] shadow-sm transition-all hover:shadow-md hover:border-[var(--border-hover,var(--primary))] group/section"
                >
                  {(section.title || section.description) && (
                    <div className="space-y-[var(--spacing-sm)] border-b border-[var(--border)]/60 pb-[var(--spacing-lg)] mb-[var(--spacing-lg)]">
                      {section.title && (
                        <h3 className="text-lg font-bold tracking-tight text-[var(--foreground)] group-hover/section:text-[var(--primary)] transition-colors">
                          {section.title}
                        </h3>
                      )}
                  ...
                  <div className="grid grid-cols-12 gap-x-[var(--spacing-lg)] gap-y-7">{section.fields.map((f, i) => renderField(f, i))}</div>
                </div>
              ))
          ...
          <div className="col-span-12 flex justify-end items-center gap-[var(--spacing-md)] mt-[var(--spacing-xl)] pt-[var(--spacing-lg)] border-t border-[var(--border)]/60">
            ...
            <Button
              ...
              loading={submitting}
              disabled={submitting}
              className={cn("min-w-[140px] shadow-lg shadow-primary/20 hover:shadow-primary/40 transition-all active:scale-95", submitClassName)}
            >
              {submitText}
            </Button>
          </div>
